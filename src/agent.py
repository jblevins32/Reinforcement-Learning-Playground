...
class Agent():
    def __init__(self, env, n_obs, n_actions, writer, **kwargs):
        ...
        if self.rl_alg.on_off_policy == "off":
            ...
            self.traj_data = TrajData(n_steps=self.t_steps, n_envs=self.num_environments, n_obs=n_obs, n_actions=n_actions, space=self.space)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            self.writer.add_scalars("reward", {self.rl_alg.name: reward_to_log}, epoch)
            self.writer.flush()
            self.writer.add_scalars("loss", {self.rl_alg.name: loss_to_log}, epoch)
            self.writer.flush()
            ...
            if ((epoch + 1) % self.save_every == 0) and epoch != 0:
                model_dir = os.path.join(root_dir,"models",f"{self.gym_model}_{self.rl_alg.name}_{reward_to_log}.pth")
    ...
    def update(self):
        ...
        if self.rl_alg.target_updates:
            for target_param, param in zip(self.rl_alg.critic_1_target.parameters(), self.rl_alg.critic_1.parameters()):
                target_param.data.copy_(self.rl_alg.tau * param.data + (1 - self.rl_alg.tau) * target_param.data)
    ...
    def rollout_cont(self, obs):
        ...
        for t in range(self.t_steps):
            ...
            actions, log_probs, dist = GetAction(self.rl_alg, obs, target=False, grad = False)
